...
export const saveHandlePeriod = async period => {
  await storageSet({ [STORAGE_KEYS.HANDLE_PERIOD]: period });
};

export const getHandlePeriod = async () => {
  const result = await storageGet([STORAGE_KEYS.HANDLE_PERIOD]);
  return result[STORAGE_KEYS.HANDLE_PERIOD] || {};
};
